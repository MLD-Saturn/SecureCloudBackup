...
| Azure SDK | `Azure.Storage.Blobs` + `Azure.Identity` (see `src/SecureCloudBackup.Storage.Azure/SecureCloudBackup.Storage.Azure.csproj` for current pinned versions). The Azure SDK lives ONLY in the `SecureCloudBackup.Storage.Azure` provider project; `SecureCloudBackup.Core` is provider-neutral and references no Azure package. |
