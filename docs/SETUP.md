...
| Local database (production) | SQLCipher-encrypted SQLite (`SQLitePCLRaw.bundle_e_sqlcipher` 2.1.x, `Microsoft.Data.Sqlite` 10.x). The bundled native SQLite is subject to CVE-2025-6965 (GHSA-2m69-gcr7-jv3q), which has **no patched package release** (2.1.11 is the latest and ships the affected engine). The NuGet audit warning is suppressed once, advisory-scoped, in the repo-root `Directory.Build.props`; the flaw needs attacker-controlled SQL and the app only runs its own parameterized statements against a local, app-owned DB. Remove the suppression and upgrade once SQLitePCLRaw ships SQLite >= 3.50.2. |
