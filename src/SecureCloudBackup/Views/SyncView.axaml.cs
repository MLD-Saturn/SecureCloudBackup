...
public partial class SyncView : UserControl
{
    ...
    private const string LocalFileDragFormat = "SecureCloudBackup.LocalFiles";
    private const string AzureFileDragFormat = "SecureCloudBackup.AzureFiles";
...
#pragma warning restore CS0618
    ...
    /// </summary>
    private static SecureCloudBackup.ViewModels.LocalFileTreeNodeViewModel? FindLocalFileNodeFromSource(Control? source)
    {
        while (source != null)
        {
            if (source.DataContext is SecureCloudBackup.ViewModels.LocalFileTreeNodeViewModel node)
                return node;
    ...
    /// </summary>
    private static SecureCloudBackup.ViewModels.FileTreeNodeViewModel? FindAzureFileNodeFromSource(Control? source)
    {
        while (source != null)
        {
            if (source.DataContext is SecureCloudBackup.ViewModels.FileTreeNodeViewModel node)
                return node;
            source = source.Parent as Control;
        }
        return null;
