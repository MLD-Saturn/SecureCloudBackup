...
public partial class MainWindowViewModel
{
        ...
        try
        {
            ...
            Progress<(int current, int total, string file, string action)> progress = new(p =>
            {
                ...
                if (string.Equals(p.action, "Deleting", StringComparison.Ordinal) && !deletePhaseReported)
                {
                    ...
                    ProgressTab.ReportProgress(new SecureCloudBackup.Core.Models.OperationProgressReport
                    {
                        Type = SecureCloudBackup.Core.Models.OperationProgressType.PhaseChanged,
                ...
                foreach (var i in candidateIndices)
                {
                    ...
                    if (isSmall)
                    {
                        ...
                        ProgressTab.ReportProgress(new SecureCloudBackup.Core.Models.OperationProgressReport
                        {
                            Type = SecureCloudBackup.Core.Models.OperationProgressType.SmallFileGroupProgress,
                    ...
                    else
                    {
                        ...
                        ProgressTab.ReportProgress(new SecureCloudBackup.Core.Models.OperationProgressReport
                        {
                            Type = SecureCloudBackup.Core.Models.OperationProgressType.FileCompleted,
                            FileIndex = i,
                            FileName = fn,
                            FileSize = filesToSync[i].FileSize,
                            FileBytesProcessed = filesToSync[i].FileSize,
                            FileStatus = SecureCloudBackup.Core.Models.FileOperationStatus.Complete,
                            TotalBytesProcessed = totalProcessed, TotalBytes = totalBytes,
                            TotalFilesCompleted = completedFileSet.Count, TotalFiles = filesToSync.Count
                        });
                    }
            ...
            Progress<(long bytesCompleted, long fileSize, int fileIndex, SecureCloudBackup.Core.Models.FileOperationStatus status)> byteProgress = new(p =>
            {
                ...
                if (isSmall && p.status == SecureCloudBackup.Core.Models.FileOperationStatus.Recovering)
                ...
                if (p.status is SecureCloudBackup.Core.Models.FileOperationStatus.Complete
                    or SecureCloudBackup.Core.Models.FileOperationStatus.Recovered
                    or SecureCloudBackup.Core.Models.FileOperationStatus.Failed)
                {
                    completedFileSet.TryAdd(p.fileIndex, 0);
                    var terminalType = p.status switch
                    {
                        SecureCloudBackup.Core.Models.FileOperationStatus.Complete => SecureCloudBackup.Core.Models.OperationProgressType.FileCompleted,
                        SecureCloudBackup.Core.Models.FileOperationStatus.Recovered => SecureCloudBackup.Core.Models.OperationProgressType.FileRecovered,
                        _ => SecureCloudBackup.Core.Models.OperationProgressType.FileFailed
                    };
                    ProgressTab.ReportProgress(new SecureCloudBackup.Core.Models.OperationProgressReport
                    {
                        Type = terminalType,
                        ...
                        FileStatus = p.status,
                        ErrorMessage = p.status == SecureCloudBackup.Core.Models.FileOperationStatus.Failed ? "Restore failed" : string.Empty,
                ...
                if (asIndividualRow && rowStartedSet.TryAdd(p.fileIndex, 0))
                {
                    ProgressTab.ReportProgress(new SecureCloudBackup.Core.Models.OperationProgressReport
                    {
                        Type = SecureCloudBackup.Core.Models.OperationProgressType.FileStarted,
                        FileIndex = p.fileIndex, FileName = fileName, FileSize = p.fileSize,
                        FileStatus = p.status,
                        TotalBytesProcessed = Volatile.Read(ref totalBytesProcessed), TotalBytes = totalBytes,
                        TotalFilesCompleted = completedFileSet.Count, TotalFiles = filesToSync.Count
                ...
                if (asIndividualRow)
                {
                    ProgressTab.ReportProgress(new SecureCloudBackup.Core.Models.OperationProgressReport
                    {
                        Type = SecureCloudBackup.Core.Models.OperationProgressType.FileProgress,
                        FileIndex = p.fileIndex, FileName = fileName, FileSize = p.fileSize,
                        FileBytesProcessed = p.bytesCompleted,
                        FileStatus = p.status,
                        TotalBytesProcessed = totalProcessed, TotalBytes = totalBytes,
                ...
                if (p.status == SecureCloudBackup.Core.Models.FileOperationStatus.Downloading && p.bytesCompleted >= p.fileSize && completedFileSet.TryAdd(p.fileIndex, 0))
                {
                    if (!asIndividualRow)
                    {
                        var sc = Interlocked.Increment(ref mirrorSmallCompleted);
    ...
    /// </summary>
    private async Task<SecureCloudBackup.Core.Models.RestoreResult> ExecuteRestoreWithRemappingAsync(
    ...
    /// </summary>
    private void LogRestoreResult(SecureCloudBackup.Core.Models.RestoreResult result)
    {
