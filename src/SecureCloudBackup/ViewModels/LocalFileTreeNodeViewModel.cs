...
public partial class LocalFileTreeNodeViewModel : TreeNodeViewModelBase<LocalFileTreeNodeViewModel>
{
    ...
    /// </summary>
    public string FileSizeText => IsFile ? SecureCloudBackup.Core.FormatHelper.FormatBytes(FileSize) : string.Empty;
    ...
    public static List<LocalFileTreeNodeViewModel> BuildTree(
        IEnumerable<SecureCloudBackup.Core.Models.WatchedFolder> watchedFolders,
        IDictionary<string, SecureCloudBackup.Core.Models.BackedUpFile> backedUpFiles)
    {
        ...
        foreach (var folder in watchedFolders.Where(f => f.IsEnabled))
        {
            ...
            LocalFileTreeNodeViewModel rootNode = new(
                SecureCloudBackup.Core.PathHelper.GetDisplayName(folder.Path),
                folder.Path,
                isFolder: true);
        ...
        List<string> excludePatterns)
    {
        try
        {
            ...
            foreach (var subDir in Directory.EnumerateDirectories(directoryPath))
            {
                ...
                if (SecureCloudBackup.Core.GlobMatcher.MatchesAny(dirName, excludePatterns)) continue;
            ...
            foreach (var filePath in Directory.EnumerateFiles(directoryPath))
            {
                ...
                // Check exclusion patterns
                if (SecureCloudBackup.Core.GlobMatcher.MatchesAny(fileName, excludePatterns)) continue;
                ...
                try
                {
                    ...
                    if (backedUpFiles.TryGetValue(filePath, out var backup))
                    {
                        ...
                        var hasValidBackup = backup.Status == SecureCloudBackup.Core.Models.BackupStatus.Completed &&
                                            !string.IsNullOrEmpty(backup.FileHash) &&
                                            backup.FileSize > 0;

                        if (backup.Status == SecureCloudBackup.Core.Models.BackupStatus.Excluded)
                        {
                        ...
                        else if (!hasValidBackup)
                        {
                            ...
                            if (backup.Status == SecureCloudBackup.Core.Models.BackupStatus.Failed && 
