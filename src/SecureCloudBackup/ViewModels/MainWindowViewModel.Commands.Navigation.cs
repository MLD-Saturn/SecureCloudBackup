...
public partial class MainWindowViewModel
{
    ...
    /// <c>%LOCALAPPDATA%\SecureCloudBackup</c> folder, pick the right daily log,
    ...
    private void ExportDiagnosticBundle()
    {
        try
        {
            ...
            var bundlePath = SecureCloudBackup.Core.DiagnosticBundleExporter.Export(
    ...
    private async Task RefreshFromRemoteAsync()
    {
        ...
        try
        {
            ...
            Avalonia.Threading.Dispatcher.UIThread.Post(() =>
            {
                ...
                TotalSize = SecureCloudBackup.Core.FormatHelper.FormatBytes(files.Sum(f => f.FileSize));
    ...
    private async Task RefreshLocalFilesAsync()
    {
        ...
        await Task.Run(() =>
        {
            ...
            {
                Dictionary<string, SecureCloudBackup.Core.Models.BackedUpFile> validatedBackups = new(StringComparer.OrdinalIgnoreCase);
