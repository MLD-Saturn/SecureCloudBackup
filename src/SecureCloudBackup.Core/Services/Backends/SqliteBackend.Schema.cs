...
using static SecureCloudBackup.Core.KdfParameters;
using static SecureCloudBackup.Crypto.Argon2idDeriver;
...
internal partial class SqliteBackend
{
    ...
    {
        // W-DB-enc Step 7: SecureCloudBackup.Core no longer ships the SQLCipher native
        ...
        // securecloudbackup-migrate helper process (which references bundle_e_sqlcipher).
        // Throw a clear managed exception so any remaining caller fails loudly
        // instead of crashing the host.
        throw new NotSupportedException(
            "SecureCloudBackup.Core no longer contains a SQLCipher engine (it uses the " +
            "CVE-fixed bundle_e_sqlite3). Open the catalog via InMemorySnapshotBackend, " +
            "or read a legacy SQLCipher database with the securecloudbackup-migrate helper process.");
    }
